    ...
    if (update.hashFork == pCoreProtocol->GetGenesisBlockHash())
    {
        for (const CBlockEx& block : boost::adaptors::reverse(update.vBlockAddNew))
        {
            ...
            for (size_t i = 0; i < block.vtx.size(); i++)
            {
                ...
                if (tx.sendTo.IsTemplate() && tx.sendTo.GetTemplateId().GetType() == TEMPLATE_FORK && !tx.vchData.empty())
                {
                    uint256 hashNewFork;
                    try
                    {
...
int64 CForkManager::ForkLockedCoin(const uint256& hashFork, const uint256& hashBlock)
{
    ...
    if (it != mapForkSched.end())
    {
        ...
        if (nHeight < 0)
        {
            StdWarn("ForkManager::CSH", "GetValidForkCreatedHeight failed");
            return -1;
        }
    ...
    StdWarn("ForkManager::CSH", "mapForkSched find failed");
    return -1;
}
...
int CForkManager::GetValidForkCreatedHeight(const uint256& hashBlock, const uint256& hashFork)
{
    ...
    if (it != mapBlockValidFork.end())
    {
        StdWarn("ForkManager::CSH", "mapBlockValidFork finded");
        ...
        {
            StdWarn("ForkManager::CSH", "second.hashRefFdBlock != 0");
    ...
    StdWarn("ForkManager::CSH", "return -1");
    return -1;
}
